...
def showSetting(request):
    ...
    context={"ProjID":Initial[0][1],"STID":Initial[0][2],"Address":Initial[0][3],"Lng":Initial[0][4],"Lat":Initial[0][5],"Area":"捷思伺服器","IP":Initial[0][6],"PORT":Initial[0][7],"Data":IsShow}
...
def GetDashboard(request):
    ...
    for i in isshowid: #[0,1, 2, 3, 4, 5, 6]
        Data.append({"Date_Time":GetData[1],"ProjID":str(GetData[2]),"STID":str(GetData[3]),"Item_name":str(Re_Value[i][11]),"Value_be":str(round(GetData[i+4+number],2)),"Value_af":str(round(GetData[i+4],2)),"a":"","b":"","offset":"","isshow":Re_Value[i][12]})
...
def SaveBackup(request):
    if request.method == "POST":
        ...
        value=DATA['value']
        DBmysql.write_many_mysql(Database,Table,column_name,value)
        return JsonResponse(DATA)

def SaveLoaclValue(request):
    if request.method == "POST":
        DATA =json.loads(request.body)
        DATA = eval(DATA)
